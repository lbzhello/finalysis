...
public class AnalysisController {
    ...
    @Operation(summary = "获取 5 日线突破十日线的股票")
    @GetMapping("5-cross-10")
    public Flux<DailyData> fiveCrossTen(
            @Parameter(description = "最大突破天数", example = "3")
            @RequestParam(name = "days", required = false) Integer days,
            @Parameter(description = "当前日期，默认数据库最新", example = "3")
            @RequestParam(name = "date", required = false) LocalDate date
    ) {
        return analysisService.fiveCrossTenDetail(Optional.ofNullable(days).orElse(3), date);
    }

    @Operation(summary = "获取 5 日线在 10 日线上方的股票的股票")
    @GetMapping("5-above-10")
    public Flux<DailyData> fiveAboveTen(
            @Parameter(description = "最小持续天数", example = "3")
            @RequestParam(name = "days", required = false) Integer days,
            @Parameter(description = "当前日期，默认数据库最新")
            @RequestParam(name = "date", required = false) LocalDate date
    ) {
